...
import { ChevronRight, Clock, MapPin, Scissors, ShoppingBag, Tag } from 'lucide-react';
...
export default function Home() {
  ...
  const homeServices = [
    ...
    { 
      ...
      descriptionKey: { en: 'Conveniently located in the heart of Kutabumi.', id: 'Berlokasi strategis di jantung Kutabumi.' }
    },
    {
      icon: <ShoppingBag className="h-8 w-8 text-primary" />,
      titleKey: { en: 'Quality Products', id: 'Produk Berkualitas' },
      descriptionKey: { en: 'We use and sell premium grooming products.', id: 'Kami menggunakan dan menjual produk perawatan premium.' }
    }
  ];
  ...
  return (
    <div className="space-y-12 animate-in fade-in-0 duration-500">
      ...
      <div className="relative w-screen -mx-4 sm:-mx-6 md:-mx-8 lg:-mx-container px-0 -mt-8 h-[60vh] min-h-[400px] max-h-[600px] mb-16">
        <Image
          key={bannerImagesData[currentBannerIndex].src} 
          ...
          sizes="100vw"
          className="-z-10 transition-opacity duration-1000 ease-in-out" 
        />
      ...
      <section className="w-full mb-16 animate-in fade-in-0 slide-in-from-bottom-10 delay-200 duration-500">
        <h2 className="font-headline text-3xl font-bold text-center mb-10 text-primary">
          {t({ en: 'What We Offer', id: 'Yang Kami Tawarkan' })}
        </h2>
          ...
          {homeServices.map((service, index) => (
            <Card key={index} className="bg-card shadow-lg hover:shadow-primary/20 transition-shadow duration-300">
              <CardHeader className="items-center">
                {service.icon}
                <CardTitle className="mt-4 font-headline text-xl text-center">{t(service.titleKey)}</CardTitle>
              </CardHeader>
        ...
        <div className="text-center mt-10">
            <Button asChild variant="link" className="text-primary hover:text-accent font-headline text-lg">
              <Link href="/services-pricing">
                {t({ en: "View All Services & Pricing", id: "Lihat Semua Layanan & Harga" })} <ChevronRight className="ml-1 h-5 w-5" />
              </Link>
            </Button>
          </div>
      </section>
