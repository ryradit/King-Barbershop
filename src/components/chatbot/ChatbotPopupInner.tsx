    ...
    try {
      ...
      if (conversationContext === 'awaiting_recommendation_details') {
        ...
        if (feedbackResult.isNegative && feedbackResult.response) {
          ...
          // If not negative or no specific feedback response, fallback to general question
          flowUsed = 'general';
          const generalAnswerResult = await answerHairstyleQuestion({ question: messageText, language });
          botResponse = generalAnswerResult.answer;
        }
    ...
    (async () => {
      ...
      try {
        if (option.action === 'init_book_appointment') {
          ...
          // Or, try feedback flow
          flowUsed = 'feedback';
          const feedbackResult = await handleCustomerFeedback({ inputText: userMessage, language, source: 'chatbot' });
          if (feedbackResult.isNegative && feedbackResult.response) {
            botResponse = feedbackResult.response;
          } else {
            flowUsed = 'general';
